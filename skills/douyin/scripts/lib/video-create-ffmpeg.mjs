...
/**
 * @param {{ text: string, voice: string, workDir: string, videoPath: string }} opts
 */
export function synthesizeTts(opts) {
  const { text, voice, workDir } = opts;
...
export function renderFfmpegAss(opts) {
  ...
  console.log('[douyin] ffmpeg ASS compositing 1080x1920...');
  run('ffmpeg', [
    '-y',
    '-f',
    'lavfi',
  ...
  return { assPath, cueCount: cues.length, renderer: 'ffmpeg-ass' };
}
