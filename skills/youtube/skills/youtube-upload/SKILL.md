...
不要假设一定能读取 uploader 源码；只有 `sau` 失败时才回退 `skills/youtube` 内置 Playwright。
...
node skills/youtube/scripts/cli.mjs publish --video "D:/..." --title "标题" --privacy unlisted
```
...
3. 执行 `sau youtube ...` 或 `skills/youtube/scripts/cli.mjs`
