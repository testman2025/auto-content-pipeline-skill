...
│   → xhs-card-render（小红书 MD→卡片图，非 AI）      │
...
| 小红书 | 标题反问/感叹，对话口吻；**配图 MD** 按 xhs-card-render 撰稿规格：每节 **220-280 字**、导语 2-3 句 + 4-5 条要点（每条 20-35 字）；全篇目标 3-4 张正文卡 |
...
| 小红书 | 输出 **配图用 MD**（YAML frontmatter + `---` 分节）；每节 **220-280 字**：标题 + 导语 2-3 句 + 4-5 条要点（每条 20-35 字） |
| 抖音 | 口播稿 + 画面提示标注 `[画面: ...]` |
...
| 平台 | 技能 | 说明 |
|------|------|------|
| **小红书正文卡片** | **xhs-card-render** | MD→HTML/CSS→PNG，`professional` 主题，**禁止 tokenware AI 生图** |
...
加载 `skills/image/skills/xhs-card-render/SKILL.md`。

MD 须含 YAML frontmatter（`emoji`/`title`/`subtitle`），章节之间用 `---` 分隔。撰稿规格见 xhs-card-render 技能（每节 120-200 字、4-5 条要点）。
...
2. **配图分平台**：小红书正文卡片走 `xhs-card-render`（禁止 tokenware AI）；知乎/公众号封面走 `tokenware-image` 或 `npm run image:generate`。
...
- [ ] Step 4c 封面/卡片图已生成（小红书 xhs-card-render / 其他 tokenware-image）或用户选择跳过
