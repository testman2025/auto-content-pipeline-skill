...
| **知乎**           | ✅ 已跑通                  | `skills/zhihu` + pyzhihu-cli         | `zhihu:login` → `zhihu:publish --content-file ...`（MD→HTML） |
| **小红书**          | ✅ 已跑通                  | skills/xiaohongshu + Chrome 扩展       | `skills/xiaohongshu/scripts/cli.py publish ...`             |
...
- **海外平台**：Agent 禁止代开浏览器、禁止连跑 `check-login`（见 `references/overseas-automation-rules.md`）
- 新平台首次发布前，须用户本人在终端确认，勿由 Agent 自动检测登录态
...
│   → pipeline:xhs（小红书配图 MD → 卡片 PNG）        │
│   → tokenware-image（知乎/公众号封面 gpt-image-2）   │
└──────────┬───────────────────────────────────────┘
           ▼
┌──────────────────────────────────────────────────┐
│  Step 5: 自动发布 + 分发                          │
...
| 平台  | 改写规则                                                                                              |
| --- | ------------------------------------------------------------------------------------------------- |
| 知乎  | 母稿微调：**重写格式**——去掉所有 `#` `##` `###` 标题语法，改加粗文字做章节标题；去掉所有 `                                         |
| 公众号 | 标题加钩子（数字/反问），开头场景切入，每3段小标题，数据故事化，金句收尾，~2500字                                                      |
| 小红书 | 标题反问/感叹，对话口吻；**配图 MD** 按 xhs-card-render 撰稿规格：每节 **260-320 字**、导语 2-3 句 + **5-6 条**要点（每条 20-35 字） |
| 抖音  | 3秒钩子开头，全程口语化，**短句排版（每句15-22字）**；`[画面:]` 可选（渲染时自动剥离），600-900字口播稿                                   |
...
| 平台              | 技能                  | 说明                                                            |
| --------------- | ------------------- | ------------------------------------------------------------- |
| **小红书正文卡片**     | **pipeline:xhs**（优先） | 配图 MD → PNG + manifest；**失败后**才降级 tokenware（见下） |
...
加载 `skills/image/skills/xhs-card-render/SKILL.md`（撰稿规格与主题说明）。
...
**配图降级**（详见 `skills/xiaohongshu/references/xhs-cron-runbook.md`）：

- **必须先** `pipeline:xhs`，禁止起手「xhs-card-render 或 tokenware」并行二选一
- **交互任务**：pipeline 失败后询问用户是否改用 tokenware AI，须告知可能被平台标记为 AI 的风险
- **定时任务（Cron）**：pipeline 失败后**自动** tokenware 降级，无需用户确认
- **禁止** BMP/ffmpeg/execute_code 自造图
...
1. 小红书：试一次 `npm run pipeline:xhs -- -Slug {slug}`；失败则 `npm run tool:install` 后重试
2. 仍失败：**交互**问用户是否 tokenware（附 AI 标记风险）；**Cron** 自动 tokenware
3. 其他平台：试一次 `tokenware-image` CLI
4. 仍失败则汇报选项：A) 检查环境/Key 重试；B) 跳过配图；C) 放弃
...
加载 **xiaohongshu-publish** skill 与 `skills/xiaohongshu/references/xhs-cron-runbook.md`。

**官方图片格式**：仅 png/jpg/jpeg/webp，单张 ≤32MB，建议 ≥720×960、3:4 宽高比。
...
python cli.py publish \
  ...
  --images "D:/test/hermes/图片/小红书/{slug}/cover.png" "D:/test/hermes/图片/小红书/{slug}/card_1.png" \
  --tags "标签1" "标签2" \
  --verify
```
...
```bash
python cli.py verify-publish --title-file /tmp/title.txt --wait-minutes 3
```

验收通过则**禁止**同标题重复发布。验收失败才 `save-draft` → 右侧草稿 → `recover-publish`；仍失败输出 `next_action: user_decision`，不关浏览器。
...
> **Agent 强制规则**（`references/overseas-automation-rules.md`）  
> 禁止自动打开浏览器 / Cursor 内浏览器访问海外登录页；禁止连续 `check-login`；禁止未经用户明确要求执行海外 publish。默认只出稿。
...
| 配图 | ✅/⏭️ | 本地路径 | xhs-card-render / tokenware-image |
```
...
| `skills/xiaohongshu/references/xhs-cron-runbook.md` | skills/xiaohongshu/ | 小红书 Cron/发布 SOP、配图降级、验收 |
| `skills/image/skills/xhs-card-render/SKILL.md` | skills/image/ | 小红书 MD→卡片图（Auto-Redbook） |
| `skills/image/skills/tokenware-image/SKILL.md` | skills/image/ | 封面生图（tokenware gpt-image-2） |
...
2. **配图分平台**：小红书**先** `pipeline:xhs` 模版卡片，失败再 tokenware 降级（交互须用户确认+AI 风险告知；Cron 自动降级）；知乎/公众号封面走 `tokenware-image`。
...
12. **海外平台禁止 Agent 代开登录**：不得自动打开 Chrome/Cursor 浏览器访问 LinkedIn/X/YouTube/TikTok/Reddit 登录页；不得连跑 check-login。见 `references/overseas-automation-rules.md`。
13. **母稿直接当公众号文章用**：母稿是知乎深度格式（3000-5000字），公众号需改写为2500字以内+场景切入+金句收尾。
14. **定时任务不确认矩阵就往下跑**：cron模式下矩阵生成后必须等待用户确认才能进入 Step 3-5。确认前最多完成选题采集。
15. **小红书标题超过20字**：平台强制限制，撰稿时控制在20字以内。
16. **小红书 CLI 超时不等于失败**：publish 报错后先 `verify-publish`，禁止盲目换图连发同标题。
17. **风控问题**：同一IP高频发布可能触发平台风控。不同平台之间间隔5-10分钟再发布。
...
- [ ] Step 4c 封面/卡片图已生成（小红书 `pipeline:xhs` / 其他 tokenware-image）或用户选择跳过
