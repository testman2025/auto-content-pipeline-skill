...
def is_markdown_heading(paragraph: str) -> bool:
    """判断段落是否为 Markdown 标题（h1-h6）。"""
    return bool(re.match(r'^#{1,6}\s+', paragraph.strip()))
...
async def _measure_content_height(page, test_md: str, theme: str, width: int,
                                  height: int) -> int:
    """测量给定 Markdown 在卡片模板中的内容高度。"""
...
async def auto_split_section(body: str, theme: str, width: int, height: int,
                             page, available_height: int) -> List[str]:
    """在单个章节内按高度自动分页，并避免标题与正文分离。"""
    ...
    for para in paragraphs:
        ...
        content_height = await _measure_content_height(
            page, test_md, theme, width, height
        )

        if content_height > available_height and current_content:
            # 标题孤儿保护：不要把章节标题留在上一张卡片末尾
            if is_markdown_heading(current_content[-1]):
                orphan_heading = current_content.pop()
                if current_content:
                    cards.append('\n\n'.join(current_content))
                current_content = [orphan_heading, para]
            else:
                cards.append('\n\n'.join(current_content))
